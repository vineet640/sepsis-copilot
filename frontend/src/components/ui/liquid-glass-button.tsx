...
/**
 * Liquid glass / metal button primitives.
 * Vite + React (no Next.js) — "use client" omitted.
 *
 * Standard app buttons remain in `@/components/ui/button.jsx`.
 */
...
/** SVG defs for `filter: url(#id)`. Avoid `display:none` — some engines won’t apply those filters. */
export function GlassFilter({ filterId }: { filterId: string }) {
  return (
    <svg className="pointer-events-none fixed h-0 w-0 overflow-hidden" aria-hidden>
      <defs>
        <filter
          id={filterId}
          x="0%"
  ...
  ({ className, variant, size, asChild = false, children, type = "button", ...props }, ref) => {
    ...
    return (
      ...
      >
        ...
        />
        <div
          className="absolute left-0 top-0 isolate -z-10 h-full w-full overflow-hidden rounded-md bg-primary/30 opacity-80 mix-blend-soft-light"
